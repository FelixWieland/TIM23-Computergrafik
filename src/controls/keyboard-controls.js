...
export class KeyboardControls {
    ...
    toggleUI() {
        ...
        const elements = [statsPanel, ...controls];
    ...
    update() {
        ...
        if (this.controls.isLocked === true) {
            ...
            if (this.moveForward || this.moveBackward)
                this.velocity.z -= this.direction.z * 100.0 * 4 * delta;
            if (this.moveLeft || this.moveRight)
                this.velocity.x -= this.direction.x * 100.0 * 4 * delta;
            if (this.moveUp)
                this.velocity.y += 100.0 * 4 * delta; // Fly up
            if (this.moveDown)
                this.velocity.y -= 100.0 * 4 * delta; // Fly down
            if (this.collisionDetector) {
                const currentPosition = this.controls.object.position.clone();
                const cameraRelativeMovement = new THREE.Vector3(-this.velocity.x * delta, // left/right
